...
@main_bp.route("/python0745x1xa/cal/<tableName>/<columnName>", methods=['GET'])
def python0745x1xa_cal(tableName, columnName):
    '''
    计算规则接口
    '''
...
@main_bp.route("/python0745x1xa/file/download", methods=['GET'])
def python0745x1xa_file_download():
    '''
    '''
    if request.method == 'GET':
...
@main_bp.route("/python0745x1xa/file/upload", methods=['POST'])
def python0745x1xa_file_upload():
    '''
    '''
...
@main_bp.route("/python0745x1xa/follow/<tableName>/<columnName>/<level>/<parent>", methods=['GET'])
def python0745x1xa_follow_level(tableName, columnName, level,parent):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/python0745x1xa/follow/<tableName>/<columnName>", methods=['GET'])
def python0745x1xa_follow(tableName, columnName):
    '''
...
@main_bp.route("/python0745x1xa/location", methods=['GET'])
def python0745x1xa_location():
    '''
...
@main_bp.route("/python0745x1xa/matchFace", methods=['GET'])
def python0745x1xa_matchface():
    '''
    baidubce百度人脸识别
    '''
    if request.method == 'GET':
...
@main_bp.route("/python0745x1xa/option/<tableName>/<columnName>", methods=['GET'])
def python0745x1xa_option(tableName, columnName):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/python0745x1xa/remind/<tableName>/<columnName>/<type>", methods=['GET'])  #
def python0745x1xa_remind(tableName, columnName,type):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/python0745x1xa/sh/<tableName>", methods=['POST'])
def python0745x1xa_sh(tableName):
    '''
    '''
    if request.method == 'POST':
        ...
        logging.warning("python0745x1xa_sh.res=====>{}".format(res))
        return jsonify(msg)

@main_bp.route("/python0745x1xa/upload/<fileName>", methods=['GET'])
def python0745x1xa_upload(fileName):
    '''
    上传接口
    '''
    if request.method == 'GET':
...
@main_bp.route("/python0745x1xa/group/<tableName>/<columnName>", methods=['GET'])
...
@main_bp.route("/python0745x1xa/value/<tableName>/<xColumnName>/<yColumnName>", methods=['GET'])
...
# 爬取数据
@main_bp.route("/python0745x1xa/spider/<tableName>", methods=['GET'])
def schema_spider(tableName):
    msg = {"code": normal_code, "msg": "success", "data": []}

    # Linux
    cmd = "cd /yykj/python/9999/spiderqa565z0x && scrapy crawl "+tableName+"Spider -a databaseName=python0745x1xa"
    # Windows
    # cmd = "cd C:\\test1\\spider && scrapy crawl " + tableName + "Spider -a databaseName=python0745x1xa"
    os.system(cmd)
